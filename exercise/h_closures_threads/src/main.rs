...
fn main() {
    ...
    let sf_vec = vec![2, 5, 1, 0, 4, 3];

    let handle_c = thread::spawn(move || {
        for msg in rx3 {
            println!("Child c thread: Received {}", msg);
        }
        println!("Child c done");
    });

    let handle_d = thread::spawn(move || {
        for msg in rx4 {
            println!("Child d thread: Received {}", msg);
        }
        println!("Child d done");
    });
